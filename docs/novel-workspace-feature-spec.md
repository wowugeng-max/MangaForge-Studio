...
- 章节流水线任务化：新增 `chapter_generation_pipeline` 运行记录，流水线步骤包括上下文包、场景卡、段落级正文、自检、修订、安全检查、入库和故事状态更新；任务中心支持查看步骤状态，并可暂停/继续该类任务。
- 场景卡确认：当前章可启动“流水线”，系统先生成场景卡并停在确认阶段；章节元数据支持直接编辑场景卡 JSON，确认后再继续正文生成。
- 生成前诊断：当前章新增“诊断”入口，会展示材料准备度、预检清单、阻塞项、补齐建议和写作圣经摘要。
- 故事记忆面板：右侧参考面板新增“故事记忆”，展示角色位置、关系、秘密、道具、伏笔、主线进度、时间线、未解冲突和角色 current_state。
- 写作圣经：后端新增写作圣经查看/保存接口，生成上下文包会读取稳定写作圣经；右侧参考面板新增“写作圣经”查看入口。
- 仿写安全解释：参考安全报告新增可解释字段，区分允许学习、谨慎学习、禁止学习层级，并给出照搬证据与改写建议。
- 参考提炼层级：知识提炼新增 `payoff_model`、`prose_syntax_profile`、`dialogue_mechanism` 等类别，强化爽点模型、文风句法和对话机制的抽象。
- 回归检查：新增 `scripts/check-novel-generation-workflow.mjs`，用于在后端运行时检查状态能力、项目、章节、写作圣经、生成诊断和运行记录接口。
