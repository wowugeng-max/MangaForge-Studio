...
本版本把小说模块从“生成一章正文”的工具升级为“可控生产流水线”。目标是同时支持参考作品仿写和无参考原创，并让每次生成都有前置检查、中间产物、审稿、修订、入库和追踪记录。

已落地能力：

- 原创孵化：在无参考作品时，可基于项目标题、题材、简介和目标读者生成世界观、角色、总纲、分卷、前 30 章章纲、写作圣经和商业定位。
- 章节生成流水线：章节目标确认、续写上下文包、场景卡生成、段落级正文生成、章节自检、二次修订、仿写安全阈值、入库版本、记忆状态机更新。
- 章节生成前置检查：生成前检查细纲、上一章结尾钩子、角色状态、本章推进点、禁止重复信息、参考知识可用性；缺关键材料时返回诊断，不直接硬生成。
- 续写上下文包：生成前把前情摘要、本章任务、角色状态、冲突目标、世界规则、参考节奏、禁止项、上一章结尾片段整理成结构化上下文。
- 章节级自检与修订：正文生成后自动检查目标完成、上章衔接、角色跑偏、设定冲突、水文重复、参考照搬和结尾钩子，并优先入库修订稿。
- 风格锁定：支持写作圣经中的叙事人称、句长、对话比例、吐槽密度、爽点密度、描写浓度、字数范围、禁用词和常用词。
- 分卷/阶段目标：写作圣经和大纲模块记录总纲、分卷目标、阶段矛盾、关键转折和章节群。
- 记忆状态机：每章入库后更新角色位置、角色关系、已知秘密、道具归属、伏笔状态、主线进度、当前时间线和重复信息。
- 仿写安全阈值：只允许迁移节奏、结构、爽点安排和信息密度；谨慎迁移人物功能和机制；禁止迁移具体桥段、专有设定、原句、角色名和核心梗。
- 仿写迁移审计：参考报告记录本次学习的节奏结构、文风密度、爽点情绪资产，以及规避项、照搬命中和改写建议。
- 生产看板：展示章节总量、已写章节、字数、分卷数量、角色数量、平均质检分、故事状态进度、失败任务和生产建议。
- 章节群任务：可创建章节群生产任务，记录起始章、数量、质量阈值、模型策略和逐章推进计划。
- 商业编辑报告：对单章生成结构、连续性、节奏、文风、原创性、商业性六维审稿，并输出必修项和一键修订提示。
- 模型调度策略：按原创孵化、大纲、场景卡、正文、审稿、修订、安全审计配置默认模型、温度、重试和 fallback 策略。
- 写作资产库：集中查看参考画像、分卷结构、章节节奏模板、角色功能矩阵、爽点模型、资源经济、文风画像、句法画像和对话机制。

当前边界：

- 原创孵化会避免覆盖同章号章节，但世界观、角色、大纲仍可能重复追加，需要后续增加去重合并。

## 1.2 商用自动写作工作台 v2

本版本补齐 v1 的闭环缺口，让工作台更接近真实生产系统。

新增能力：

- 章节群自动执行：章节群任务可以在任务中心执行，服务端会逐章执行场景卡、正文、质检、修订、安全审计、入库和状态机更新；单章失败会记录失败原因，并按任务策略暂停或继续。
- 章节群暂停继续：章节群运行记录支持暂停、继续和再次执行，当前进度、每章状态、评分、失败原因都会保存在 run output 中。
- 原创孵化人工确认：原创孵化先生成预览，用户确认核心卖点、角色和章节方向后再写入项目资料，避免初始方向偏差直接污染项目。
- 写作圣经表单化编辑：工作台顶部提供写作圣经入口，可编辑读者承诺、世界规则、主线、分卷计划、风格锁定、仿写安全策略和禁止项。
- 阶段模型调度接入：原创孵化、场景卡、正文初稿、审稿、修订、故事状态机更新、编辑报告和全书总检按 model_strategy 的阶段模型和温度执行。
- 编辑报告一键修订：右侧编辑报告可直接触发修订稿生成，修订结果入库为新版本，并保存编辑修订记录。
- 多参考融合控制台：参考工程总览显示各参考作品的权重、学习维度、节奏/文风/爽点/角色功能资产覆盖、维度冲突和照搬风险。
- 故事状态机人工校正：右侧故事记忆支持打开 JSON 校正面板，保存后后续生成优先读取人工校正后的状态机。
- 全书总检：工作台顶部提供全书总检，检查主线停滞、角色成长、伏笔回收、爽点密度、重复信息、分卷目标和写作圣经偏离。

当前边界：

- 章节群自动执行在一次 HTTP 请求内顺序处理，适合当前本地工作台；后续如果需要无人值守长任务，可迁移到持久化队列 worker。
- 写作圣经和故事状态机编辑采用 JSON 表单，已经能控制数据，但还不是完全字段化的专用表单。
- 原创孵化会避免覆盖同章号章节，但世界观、角色、大纲仍可能重复追加，需要后续增加去重合并。

## 1.3 商用自动写作工作台 v3

本版本增加生产治理层，目标是让自动写小说不只是“能生成”，而是能被长期运营、评估、复盘和调优。

新增能力：

- 后台任务队列语义：项目提供 run queue 总览，集中查看 ready/running/paused 的章节群、流水线、基准测试和全书审稿任务；本地版通过可恢复 run 记录和 execute endpoint 驱动执行。
- 成本质量仪表盘：统计章节数、已写章节、总字数、运行次数、估算 token、耗时、失败率、fallback 次数、安全拦截次数、阶段统计和产能指标。
- 人工审批关卡：项目级 approval_policy 支持全自动、半自动和严格审核策略，可控制场景卡、初稿、低分、自检和安全报告是否需要人工确认。
- 项目质量基准测试：生成质量基线报告，记录样本章节、模型策略、平均质检分、成本基线和调优建议，用于后续模型/提示词 A/B 对比。
- 章节版本评审：当前章可对比最近版本和当前正文，输出段落变更、长度变化、相似度和复核建议。
- 原创选题验证：从目标读者、商业定位、卖点、前 10 章追读风险、竞争风险和多方向方案评估原创项目。
- 章节相似度检测：对当前章执行参考项目相似度报告，检查专名/证据词命中、照搬安全、原创性和结构相似风险。
- 滚动规划器：基于写作圣经、状态机、大纲和近期审稿，生成未来 10 章规划、分卷剩余目标、伏笔回收、角色成长节点和风险提示。
- Agent 提示词配置后台：项目级 agent_prompt_config 可读取和保存版本，支持后续把硬编码提示词逐步迁移成可配置模板。
- 商业工具箱：工作台顶部集中入口，统一触发队列、指标、审批、基准、版本评审、相似度、选题验证、滚动规划、多参考融合、Agent 配置、写作圣经和状态机校正。

当前边界：

- run queue 当前是本地可恢复任务队列语义，还不是独立 worker 进程；长任务仍由前端或调用方触发 execute endpoint。
- 成本统计优先读取模型 usage；如果供应商未返回 usage，则只能统计运行次数、耗时、失败率和产能。
- Agent 提示词配置已经有项目级存储和版本号，旧 Agent 尚未全部读取该配置覆盖默认 prompt。
