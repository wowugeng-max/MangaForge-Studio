...
日期：2026-08-15（v1.4 修订 2026-08-19）  
状态：待用户审阅（v1.4，对齐 A/D/C 落地：质检 UI、后置补丁、扩纲运行时、写章运行时）  
前置：
...
4. **续写 / 回炉 / 适配仍未做。** `write_continue` / `rewrite_chapter` / `adapt_pack` 仍 `CONTRACT_NOT_IMPLEMENTED`。`generateChapterForGroup` / batch 仍走旧 API，未 cutover。画布 `prompt` 仍否（B）。
...
3. **不要**把 `oh-story-core.story-long-write.outline` 标成可执行（仍 `implemented=false`）。
4. **写章仍未做。** `write_chapter` / `write_continue` / `rewrite_chapter` / `adapt_pack` 仍 `CONTRACT_NOT_IMPLEMENTED`。替换 `generateChapterForGroup` 必须另开 spec。画布 `prompt` 仍否（B）。（**已被 v1.4 覆盖：`write_chapter` 已落地**；续写/回炉/适配与 `generateChapterForGroup` cutover 仍未做。画布 `prompt` 仍否（B）。）
...
`subject_type`：`chapter` | `project` | `outline` | `canvas_node` | `pack`。已实现：`chapter`（审稿/去AI/改稿/写本章）、`project`（开书/扩纲）。`pack` / 画布仍拒绝执行。
...
| `executeNovelAgent` 生成正文 / 大纲向导 | 深度孵化已改走 `open_book`；扩纲已走 `expand_outline`（无工作台按钮）。作者写本章走 `write_chapter`；batch / `generateChapterForGroup` 仍旧 API |
...
- 不在本文件实现画布切内核、自动改大纲。开书 `open_book`、扩纲 `expand_outline`、写章 `write_chapter` 已由动词 spec 落地（扩纲无工作台按钮；写章是作者入口，batch 未切）。
- 不删除写作 skill 市场或指纹库。
...
| 6 outline / 画布 prompt | **未做完** | 开书、扩纲、写章已落地；画布 `prompt` 仍须另开 brainstorm（B） |
...
| 3 | **C 动词 4+** | **扩纲、写章已落地**（扩纲 `2026-08-18-expand-outline-runtime`，无工作台按钮；写章 `2026-08-19-write-chapter-runtime`，作者入口） | 续写/回炉/适配仍未开计划；`generateChapterForGroup` 未 cutover |
...
- 运行时：`expand_outline` **已落地**（无工作台按钮）。`write_chapter` **已按** `2026-08-19-write-chapter-runtime` **落地**（作者入口）。未做：`write_continue` / `rewrite_chapter` / `adapt_pack`
...
- **A 内核 UI 已按** `2026-08-18-kernel-job-ui` **落地**；**D 后置补丁已按** `2026-08-18-kernel-d-patches` **落地**；**C 扩纲运行时已按** `2026-08-18-expand-outline-runtime` **落地**；**C 写章运行时已按** `2026-08-19-write-chapter-runtime` **落地**。B（画布 `prompt`）仍须另开 brainstorm。续写/回炉/适配仍须另开。
