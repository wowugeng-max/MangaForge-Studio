...
真实模型验收后的最终审查确认，字数精确命中不能凌驾于成文质量和长篇连续性之上，但也不能成为绕过质量门禁的理由。收口实现采用以下固定边界：

1. 配置字数区间仍是首选硬目标；区间上下各增加 5% 的质量软带。标准章 3,200-5,200 字对应约 3,040-5,460 字。超出软带继续失败关闭。
2. 软带只免除继续为少量字数偏差重写，不免除任何其他门禁。正文必须完整、无截断和工程泄漏，并通过六维 LLM 质量审查、确定性硬检查、核心承诺、人物行为、事件因果、上一章承接和长线状态一致性检查，才可入库。
3. 落入软带后不再触发字数修复循环；未落入软带时沿用现有有限次数，压缩最多 3 轮、扩写默认最多 3 轮且绝不超过 5 轮。达到上限仍不合格即明确失败，不继续重跑。
4. 正文核心 required sections 不得在 48,000 字符编译器之前按字段或区块静默截断。若完整 required 内容超过预算，返回 `PROSE_CORE_PROMPT_BUDGET_EXCEEDED`，由上游修材料，不带残缺核心事实生成。
5. 未显式传 `quality_threshold` 时，统一服务使用项目 `reference_config.quality_gate.min_score`，缺省再使用 78；近期交稿质量退化时仍只能提高到至少 85，不能回落到 0。
6. 初稿、失败修订和未通过安全检查的候选不得写入 Memory Palace 或其他长期事实。只有通过全部门禁并已写入章节的同一最终正文，才写一次章节版本、故事状态和长期记忆；长期记忆写入失败不得替换为初稿或触发重新生成正文。

## 15. 最终收敛验证

2026-07-11 完成收口修复与鲜活回归，未降低任何正文质量、核心承诺、连续性或安全门禁：

- 标准章 3,200-5,200 字采用对称 5% 质量软带，3,040/5,460 为边界通过值，3,039/5,461 继续失败；软带只停止字数改写，后续确定性和六维语义门禁仍完整执行。
- 缺省质量阈值按请求 snake/camel、项目 quality gate snake/camel、固定 78 的顺序取首个正数；近期交稿质量退化仍提升到至少 85。
- required prompt 在 48,000 字符编译器前不再受 4,200 字符、200 项或 24 层的隐性截断；预算内完整保留，required 总量超预算明确返回 `PROSE_CORE_PROMPT_BUDGET_EXCEEDED`。optional risk contracts 的有界表示和 director 最多四项保持不变。
- 安全批次请求的 `next_batch_brief.chapters` 不再被诊断清洗规则误删；逐章只把 `chapter_no`、标题、章节任务、冲突、章末钩子和主线推进写入 required `request_batch_role`，嵌套正文、原始载荷、prompt、消息和调试诊断继续递归剔除。没有批次章节时不输出空容器，避免无意义改变既有真实验收 prompt。
- 正文生成保留 Memory Palace 写前召回，但初稿、空输出重试稿和失败修订均不写长期记忆；章节落盘后只尝试写同一 `finalText` 一次，记忆写入异常不会触发重生成、正文替换或故事状态重复执行。
- 入库判定保持合取关系：正文须落在配置区间或 5% 软带内，同时通过确定性硬检查、六维独立复检、核心承诺、人物行为、事件因果、上一章承接、长线状态和参考安全门禁。任一必需条件失败，或最多两轮正文修订耗尽，均明确失败且不写章节、故事状态或长期记忆；不会为追求字数或审计字段继续循环生成。
- 鲜活验证结果：10 文件聚焦套件 `1710 pass / 0 fail`；完整小说服务回归 `1838 pass / 0 fail`；验证器纯函数与 SSE 合同 `26 pass / 0 fail`；合同合并与 prompt 编译两文件 `20 pass / 0 fail`；重构边界检查通过；服务构建成功打包 785 个模块。
- 已提交真实报告继续是成文质量权威证据。该次输入的 handoff、scene-causality、gate 和 core-promise 分别为 411、2,739、1,018 和 3,542 字符，均未触及旧字段上限；只读上下文复核同时确认该次请求没有安全批次章节数组，最终实现也不会输出空 `chapters` 容器，因此最终 prompt 边界与成功报告一致。验收器在第 10 章已经入库时会在备份和模型请求前拒绝覆盖，收口不再发起重复整章生成循环。
- 当前真实工作区复核：第 1-9 章与报告 `9/9` 哈希一致，第 10 章仍为 5,121 字；`workspace/providers.json` SHA-256 仍为 `205e508c14a5c7285924653aea2072f7301e52d86c24c2edd5d8cfc7c185b034`；最终 worktree 的 project 1 Memory Palace 为 0 memory / 0 fact，无需数据清理。
