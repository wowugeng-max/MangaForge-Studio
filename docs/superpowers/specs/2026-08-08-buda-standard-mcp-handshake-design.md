...
采用方案 A：删除 Buda 专用的 initialized-notification 抑制 Transport，所有 Streamable HTTP MCP Server 统一使用 SDK 原生 `StreamableHTTPClientTransport`。
...
生产代码只修改 MCP Client Transport 选择：

- 删除 `BudaStreamableHTTPClientTransport`。
- `defaultSdkFactory.createTransport` 不再检查 `adapter_id`，始终创建标准 `StreamableHTTPClientTransport`。

测试代码只调整和补充握手契约：

- 将原“Buda 不发送 initialized 通知”测试改为“Buda 使用标准 initialized 通知”。
- 断言 Buda 与 provider-neutral 服务均使用 SDK 原生 Transport。
...
5. Client 执行 `tools/list` 并缓存有界、脱敏后的工具描述。
6. Adapter 按现有逻辑解析工具并执行 Agent、Drive、Session 操作。
...
1. Buda Server 使用原生 `StreamableHTTPClientTransport`。
2. Buda 握手方法顺序为 `initialize`、`notifications/initialized`、`tools/list`。
3. initialized 通知得到成功响应后，连续两个只读工具调用都成功。
4. provider-neutral MCP 的标准握手测试继续通过。
5. Bearer Header、工具 allowlist、响应体预算、SSE 预算和秘密脱敏测试继续通过。
6. 精确 `server_not_initialized` 证据继续被安全投影，远端文本不进入错误响应。
7. Buda Adapter、Runtime、GenerationSource 与完整 Server/Web 回归继续通过。
...
- Focused MCP Client 测试先因旧 Buda Transport 行为失败，修改生产代码后转绿。
