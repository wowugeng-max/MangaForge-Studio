...
- `git diff --check` 已通过
- `bun run smoke:novel:local` 已通过，且 smoke 已覆盖前30章留存诊断、留存修复队列、300万字长线压力测试、未来100章骨架检查、骨架应用、骨架章节群 dry-run、长线生产趋势报表、长线生产修复队列、单项任务状态回写、批量状态回写与闭环审计摘要端点

### 仍待推进
- 当前两个诊断是本地规则版，适合快速筛查；后续可以增加 AI 编辑复核版，用模型给出更细的章节重写方案
- 前30章留存修复队列已能进入编辑报告/修订稿链路；后续可补“按任务类型直接重做场景卡/章末钩子”的更细分自动化
- 长线生产治理已能生成闭环摘要；后续需要把摘要接入自然语言创作指令台和项目总览

### 下一步动作
1. 将长线治理闭环摘要接入自然语言创作指令台和项目总览
2. 为留存修复任务增加更细的自动化动作，例如重做场景卡、强化章末钩子、补动作链
