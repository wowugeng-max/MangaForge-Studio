...
export function buildStoryStatePrompt(project: any, contextPackage: any, chapterText: string) {
  ...
  return [
    ...
    'established_events: array，每项包含 kind, subject, predicate, fact, cause, mechanism, constraints, aliases, source_excerpt, confidence, tags。',
    '只抽取正文明确发生、后续章节复述时不能改写的事件级正史：优先 death/injury/rule_trigger/ability_cost/identity_reveal/item_transfer/promise/secret_known。',
    '每条 established_events 必须带 source_excerpt（本章原句）；没有原文证据就不要输出。',
    'death 与 rule_trigger 尽量给 high confidence（>=0.85）并写清 cause/mechanism/constraints；不要把猜测或未发生的可能未来写成正史。',
    'character_updates: array，每项包含 name,current_state,source_excerpt 或 evidence。若正文出现尚未建卡的具名角色（含职位+姓名，如局长秦建国），也必须输出该角色的 character_updates；系统会自动建卡。禁止把已确立专名改成新名。current_state 可包含 age, location, physical_condition, appearance_delta, outfit, items, item_changes, ability_status, resource_status, emotional_state, relationship_attitudes, knowledge_scope, newly_learned, information_boundaries, secrets_known, injuries, goals, next_intent, last_seen_chapter；source_excerpt/evidence 必须引用本章正文中支撑该状态变化的原句。',
