...
test('wires source-authoritative material repair through context before prose task begin', () => {
  ...
  expect(createSource).toContain('const repairChapterMaterials = ctx.repairChapterMaterials || materialRepairService.repairChapterMaterials')
  expect(createSource).toContain('const injectedResolverLeases = ctx.generationSourceResolver?.chapterSourceLeases')
  expect(createSource).toContain('const chapterSourceLeases = injectedResolverLeases')
  expect(createSource).toContain('withChapterAuthorityFence: createChapterAuthorityFence({')
  expect(createSource).toContain('chapterSourceLeases,\n      readProject: ctx.getProject,')
  ...
  expect(contextSource).toContain('await readExpectedAuthoritySource()')
  expect(contextSource).toContain('const activeGenerationSource = (await readExpectedAuthoritySource()).active')
  expect(groupSource.slice(contextCall, proseBegin)).toContain('expectedAuthorityFingerprint,')
  expect(groupSource.slice(proseBegin)).toContain('expectedAuthorityFingerprint,')
...
test('uses the compiled generation contract for the actual draft runtime call', () => {
  ...
  expect(draftCallStart).toBeGreaterThan(contractStart)
