...
describe('chapter pre-draft brief sync-core a b', () => {
  test('asks prose revision to preserve core direction and only repair evidenced findings', () => {
    ...
    const monofileSource = ['paragraph-prose-context.ts','paragraph-prose-context-prepare.ts','paragraph-prose-context-prepare-default-five.ts','paragraph-prose-context-prepare-batch-memory.ts','paragraph-prose-context-sections.ts'].map((name) => readFileSync(join(import.meta.dir, '../novel-writing-service/service', name), 'utf8')).join('\n')
