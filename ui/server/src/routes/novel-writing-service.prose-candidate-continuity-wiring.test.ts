...
describe('prose candidate continuity wiring', () => {
  test('guards editor, meme polish, and quality revision replacements with the shared continuity-safe selector', () => {
    ...
    const qualityLoopSource = readFileSync(join(import.meta.dir, '../novel-writing/prose-quality-loop-run.ts'), 'utf8')
    ...
    expect(polishSource.match(/selectContinuitySafeProseCandidate\(chapterText, fingerprintSelection\.text, contextPackage,/g)?.length || 0).toBe(2)
