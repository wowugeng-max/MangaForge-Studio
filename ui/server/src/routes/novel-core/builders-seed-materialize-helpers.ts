...
export function getSeedRaw(seed: any) {
  const raw = parseNestedSeed(seed?.raw_payload)
  return Object.keys(raw).length ? raw : parseNestedSeed(seed)
}

export function firstNonEmptySeedArray(...values: any[]) {
  return values.find(value => Array.isArray(value) && value.length > 0) || []
}

export function getSeedChapterOutlines(seed: any) {
...
export function getSeedVolumeOutlines(seed: any) {
  const raw = getSeedRaw(seed)
  return firstNonEmptySeedArray(
    raw.volume_outlines,
    raw.volumes,
...
export function normalizeChapterSeed(chapter: any, index: number, seed: any = {}) {
...
export function buildFallbackWritingBible(seed: any, project: any = {}) {
...
export function buildMaterializedSeedCharacters(seed: any) {
  const root = parseNestedSeed(seed)
...
export function buildProjectSeedStoryState(seed: any, project: any, characters: any[]) {
...
export function normalizeSeedSceneCards(chapter: any, characters: any[]) {
