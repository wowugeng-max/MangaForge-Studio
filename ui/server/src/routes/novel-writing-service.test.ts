...
// normalizeSceneCardsPayload suites moved to novel-writing-service.scene-cards.test.ts


// chapter prose word target suites moved to novel-writing-service.prose-word-target.test.ts


// chapter pre-draft brief suites moved to novel-writing-service.pre-draft-brief.test.ts


// readability and restrained meme workflow suites moved to novel-writing-service.readability-meme.test.ts


// post-delivery sync report suites moved to novel-writing-service.post-delivery-sync.test.ts
...
describe('chapter context word target source guards', () => {
  ...
  test('keeps delivery risk receipts from prose self review', () => {
    ...
    expect(source).toContain('reviewPayload?.delivery_risk_receipts')
  ...
  test('asks prose self review and revision to enforce write-preparation execution checks', () => {
    ...
    expect(reviewNormalizeBlock).toContain('reviewPayload?.write_preparation_checks')
  ...
  test('asks prose self review and revision to enforce chapter handoff checks', () => {
    ...
    expect(reviewNormalizeBlock).toContain('reviewPayload?.chapter_handoff_checks')
  ...
  test('asks prose generation self review and revision to enforce scene-card serial risk repairs', () => {
    ...
    const reviewFieldList = source.slice(
      source.indexOf('const STRUCTURED_REVIEW_CHECK_FIELDS ='),
      source.indexOf('export function hasFailingReviewChecks'),
    )
