...
describe('chapter context handoff b', () => {
  ...
  test('draft review quality decision excludes post-delivery receipt sync advisories from the hard gate', () => {
    ...
    expect(groupSource).toContain('const qualityLoopState = await runQualityLoopPhase')
    expect(groupSource).toContain('const qualityPrestoreResult = await runQualityPrestoreFinalize')
