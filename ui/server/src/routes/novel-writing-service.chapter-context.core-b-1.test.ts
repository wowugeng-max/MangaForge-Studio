...
describe('chapter context word target source guards 1', () => {
  ...
  test('passes word-target expansion blueprint patches into prose review context', () => {
    ...
    const helperSource = ['core-handoff-sync-reports.ts','core-handoff-sync-reports-core.ts','core-handoff-sync-reports-extended.ts'].map((name) => readFileSync(join(import.meta.dir, '../novel-writing-service/post-delivery', name), 'utf8')).join('\n')
  ...
  test('does not fail chapter production solely because a recovered draft result still has an error field', () => {
    ...
    const draftStart = source.indexOf('const resultPayload = Array.isArray')
...
${selfReviewSource}`
  ...
  test('runs commercial editor rewrite between word-target expansion and self-review in chapter group generation', () => {
    const source = readFileSync(join(import.meta.dir, '../novel-writing-service/service/generate-chapter-editor-meme-polish.ts'), 'utf8')
    ...
    const wordTargetStart = source.indexOf('const wordTargetCheck = await ensureProseMeetsWordTarget')
    ...
    expect(editorStart).toBeGreaterThan(editorStageStart)
    expect(editorPipelineStart).toBeGreaterThanOrEqual(0)
    expect(reviewStart).toBeGreaterThan(editorPipelineStart)
  })
