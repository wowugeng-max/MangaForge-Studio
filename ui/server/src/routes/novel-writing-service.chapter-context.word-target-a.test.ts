...
describe('chapter context word-target a', () => {
  ...
  test('fails visibly when the authoritative prose quality review request fails', () => {
    const source = readFileSync(join(import.meta.dir, '../novel-writing-service/service/generate-chapter-quality-review-executor.ts'), 'utf8')
    const reviewStart = source.indexOf('const review = async')
    const reviewCallStart = source.indexOf('result = await executeChapterStage', reviewStart)
    const reviewEnd = source.indexOf('const payload = getNovelPayload(result)', reviewCallStart)
    const reviewBlock = source.slice(reviewStart, reviewEnd)

    expect(reviewStart).toBeGreaterThanOrEqual(0)
    expect(reviewCallStart).toBeGreaterThan(reviewStart)
    expect(reviewEnd).toBeGreaterThan(reviewCallStart)
    expect(reviewBlock).toContain('if (result?.error)')
    expect(reviewBlock).toContain("stage === 'quality_recheck' ? 'PROSE_QUALITY_RECHECK_UNAVAILABLE' : 'PROSE_REVIEW_FAILED'")
  ...
  test('feeds quality gate failure reasons into the oh-story revision strategy brief', () => {
    ...
    const monofileSource = ['prose-self-review-methods.ts','prose-self-review-prompts.ts','prose-self-review-policy.ts','prose-self-review-run-deterministic.ts','prose-self-review-run-normalize.ts','prose-self-review-run-revision.ts','prose-self-review-run.ts'].map((name) => readFileSync(join(import.meta.dir, '../novel-writing-service/service', name), 'utf8')).join('\n')
  ...
  test('passes compact previous chapter handoffs into prose draft generation', () => {
    ...
    const draftCallStart = source.indexOf('const draftResult = await chapterTaskExecution.generateDraft', prevChaptersStart)
  ...
  test('checks abort signal between expensive chapter prose pipeline stages', () => {
    ...
    expect(source).toContain('const compiledPrompt = compileParagraphProseContext')
