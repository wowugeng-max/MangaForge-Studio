...
function nextTableId(db: Database, table: string): number {
  return Number((db.query(`SELECT COALESCE(MAX(id), 0) + 1 AS id FROM ${table}`).get() as any)?.id || 1)
}

function insertProjectRow(db: Database, p: NovelProjectRecord) {
...
function updateProjectRow(db: Database, p: NovelProjectRecord) {
...
function insertWorldbuildingRow(db: Database, w: NovelWorldbuildingRecord) {
...
function updateWorldbuildingRow(db: Database, w: NovelWorldbuildingRecord) {
...
function insertCharacterRow(db: Database, c: NovelCharacterRecord) {
...
function updateCharacterRow(db: Database, c: NovelCharacterRecord) {
...
function insertOutlineRow(db: Database, o: NovelOutlineRecord) {
...
function updateOutlineRow(db: Database, o: NovelOutlineRecord) {
...
function insertChapterRow(db: Database, c: NovelChapterRecord) {
...
function updateChapterRow(db: Database, c: NovelChapterRecord) {
...
function insertChapterVersionRow(db: Database, v: NovelChapterVersionRecord) {
...
function insertSettingEntityRow(db: Database, s: NovelSettingEntityRecord) {
...
function updateSettingEntityRow(db: Database, s: NovelSettingEntityRecord) {
...
function insertChapterSettingUsageRow(db: Database, u: NovelChapterSettingUsageRecord) {
...
function updateChapterSettingUsageRow(db: Database, u: NovelChapterSettingUsageRecord) {
...
function updateRunRow(db: Database, r: NovelRunRecord) {
...
async function withNovelDbWrite<T>(activeWorkspace: string, writer: (db: Database) => T, operation = 'mutation'): Promise<T> {
  return withNovelWorkspaceMutation(activeWorkspace, async () => {
...
function createChapterVersionRecord(data: Partial<NovelChapterVersionRecord> & { id?: number }): NovelChapterVersionRecord { return { id: Number(data.id || 0), chapter_id: Number(data.chapter_id || 0), project_id: Number(data.project_id || 0), version_no: Number(data.version_no || 1), chapter_text: String(data.chapter_text || ''), scene_breakdown: toAnyArray(data.scene_breakdown), continuity_notes: toStringArray(data.continuity_notes), source: data.source || 'manual_edit', created_at: String(data.created_at || nowIso()) } }
function nextChapterVersionNo(db: Database, chapterId: number): number {
...
function loadAcceptanceWorkingSet(db: Database, chapterId: number): { store: NovelStore; chapterIndex: number; projectIndex: number } | null {
  const chapterRow = db.query('SELECT * FROM chapters WHERE id = ? LIMIT 1').get(chapterId) as any
