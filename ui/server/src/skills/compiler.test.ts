...
describe('prompt compiler', () => {
  ...
  test('redacts secrets and internal fields in every text model-bound part', async () => {
    ...
    await compiler({ skillName: 'h3', rawPrompt: `${root}/private sk-test-secret-token activeWorkspace`, mode: 'text_to_video', incomingAssets: [{ type: 'prompt', content: 'Bearer abc' }, { type: 'image', url: `https://x/a.png?token=secret&workspace=${encodeURIComponent(root)}` }], nodeParams: { cameraParams: { api_key: 'sk-another-secret', activeWorkspace: root } }, arguments: { style: `${root}/style` }, activeWorkspace: root, compilerModelId: 10 })
