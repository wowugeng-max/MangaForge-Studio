...
function benchmarkRecallBriefFromContext(contextPackage: any, chapter: any = {}) {
  const target = {
...
function benchmarkRecallBeat(key: string, label: string, value: any) {
  const text = compactText(value, 220)
  return text ? { key, label, text } : null
}

function benchmarkRecallBeatMatch(beat: any, chapterText: string) {
...
function benchmarkRecallHardGapMisses(brief: any = {}) {
...
function styleBoundarySyncMiss(key: string, label: string, text: string, evidence: string, fix: string) {
  return {
    key,
    label,
...
function chapterHookContractForSync(contextPackage: any = {}, chapter: any = {}) {
  const target = contextPackage?.chapter_target || {}
  const brief = {
    ...(target?.pre_draft_brief || {}),
    ...(target?.preDraftBrief || {}),
...
function buildChapterEndingContractCheck(contextPackage: any, chapterText: string) {
...
function contextWithChapterRawPreDraftForSync(contextPackage: any = {}, chapter: any = {}) {
...
function paragraphHookContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
...
function suspenseContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
  return buildSuspenseContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter))
}

function normalizeSuspenseExpectationChainContract(value: any) {
...
function suspenseExpectationLineMentioned(line: string, chapterText: string) {
  const text = String(chapterText || '')
...
function normalizeSuspenseExpectationChainCheck(value: any, expectationLayers: any, chapterText: string) {
...
function normalizeSuspenseForeshadowingBoundaryCheck(values: any, chapterText: string) {
...
function buildSuspenseDeterministicCheck(chapterText: string) {
  const risks = [
...
function reversalContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
  return buildReversalContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter))
}

function buildReversalDeterministicCheck(chapterText: string) {
  const text = String(chapterText || '')
  const risks = [
...
function showdownContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
  return buildShowdownContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter)) || {}
}

function buildShowdownDeterministicCheck(chapterText: string) {
  const risks = [
    ...scanShockLayeringRisks(chapterText),
...
function showdownExplicitRuleKeys(contextPackage: any = {}) {
...
function bridgeUnitContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
  return buildBridgeUnitContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter)) || {}
}

function buildBridgeUnitDeterministicCheck(chapterText: string) {
  const risks = [
...
function openingContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
  return buildOpeningContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter)) || {}
}

function buildOpeningForbiddenCheck(project: any, contextPackage: any, chapterText: string) {
  const risks = [
    ...scanOpeningHookRisks(chapterText),
    ...scanOpeningFirst50ConflictRisks(chapterText),
    ...scanOpeningEventDensityRisks(chapterText),
...
function normalizeProseCraftExpressionCheck(values: any[], chapterText: string) {
  const planned = proseCraftArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeProseCraftSceneWeavingCheck(values: any[], chapterText: string) {
  const planned = proseCraftArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeProseCraftRhythmCheck(values: any[], chapterText: string) {
  const planned = proseCraftArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeProseCraftObjectNumberCheck(values: any[], sceneAnchors: any[], chapterText: string) {
...
function normalizeProseCraftSectionStructureCheck(values: any[], chapterText: string) {
  const planned = proseCraftArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeProseCraftDensityCheck(sectionRules: any[], antiPaddingRules: any[], chapterText: string) {
...
function normalizeProseCraftConceptAnchorCheck(values: any[], contextPackage: any, chapterText: string) {
  const planned = proseCraftArray(values)
  if (!planned.length) return null
...
function buildProseCraftDeterministicCheck(contextPackage: any, chapterText: string) {
  const text = String(chapterText || '')
  const localRisks = [
...
function proseCraftPriority(missed: any[]) {
...
function punctuationToneContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
  return buildPunctuationToneContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter)) || {}
}

function punctuationToneArray(...values: any[]) {
  return uniqueBriefStrings(values.flatMap(value => asArray(value)).map((item: any) => compactBriefText(item)).filter(Boolean), 20)
}

function normalizePunctuationToneMapCheck(values: any[], scenePlan: any[], chapterText: string) {
...
function normalizePunctuationForbiddenCheck(values: any[], chapterText: string) {
...
function normalizePunctuationQuestionCheck(values: any[], chapterText: string) {
...
function normalizePunctuationExclaimCheck(values: any[], chapterText: string) {
...
function buildPunctuationToneDeterministicCheck(chapterText: string) {
  const risks = [
...
function punctuationTonePriority(missed: any[]) {
...
function qualityAuditContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
  return buildQualityAuditContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter)) || {}
}

function qualityAuditArray(...values: any[]) {
  return uniqueBriefStrings(values.flatMap(value => asArray(value)).map((item: any) => compactBriefText(item)).filter(Boolean), 20)
}

function normalizeQualityAuditStructureCheck(values: any[], chapterText: string) {
...
function normalizeQualityAuditChapterPurposeCheck(values: any[], chapterText: string) {
  const planned = qualityAuditArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeQualityAuditProgressionCheck(values: any[], chapterText: string) {
  const planned = qualityAuditArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeQualityAuditInformationCheck(values: any[], contextPackage: any, chapterText: string) {
  const planned = qualityAuditArray(values)
  if (!planned.length) return null
...
function normalizeQualityAuditEventContentCheck(values: any[], chapterText: string) {
  const planned = qualityAuditArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeQualityAuditFiveDimensionCheck(values: any[], chapterText: string) {
  const planned = qualityAuditArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeQualityAuditSellingPointExpressionCheck(values: any[], chapterText: string) {
  const planned = qualityAuditArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function buildQualityAuditDeterministicCheck(contextPackage: any, chapterText: string) {
  const risks = [
...
function qualityAuditPriority(missed: any[]) {
...
function dialogueContractForSync(contextPackage: any = {}, chapter: any = {}) {
  return buildDialogueContract(contextWithChapterRawPreDraftForSync(contextPackage, chapter)) || {}
}

function buildDialogueFunctionalFillerCheck(chapterText: string) {
...
function buildDialogueDeterministicCheck(chapterText: string) {
  const risks = [
...
function dialoguePriority(missed: any[]) {
...
function characterBehaviorContractForSync(contextPackage: any = {}, chapter: any = {}) {
...
function normalizeCharacterBehaviorAnchorCheck(key: string, label: string, values: any[], chapterText: string, fix: string, threshold = 28) {
...
function normalizeCharacterBehaviorMemoryAnchorCheck(values: any[], chapterText: string) {
  const planned = characterBehaviorArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function assetLinkageContractForSync(contextPackage: any = {}, chapter: any = {}) {
...
function normalizeAssetLinkageKeyAssetsCheck(values: any[], chapterText: string) {
...
function normalizeAssetLinkageThreeAppearanceCheck(values: any[], chapterText: string) {
  const planned = assetLinkageArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function normalizeAssetLinkageIsolationCheck(contract: any, chapterText: string) {
  const text = String(chapterText || '')
...
function relationshipGraphRiskAssetName(risk: string) {
  return compactBriefText(String(risk || '').split(/[：:]/)[0].replace(/\([^)]*\)|（[^）]*）/g, ''))
}

function relationshipGraphRiskType(risk: string) {
  const match = String(risk || '').match(/\(([^)]+)\)|（([^）]+)）/)
  return compactBriefText(match?.[1] || match?.[2] || '')
}

function normalizeAssetLinkageRelationshipGraphRiskCheck(contract: any, chapterText: string) {
...
function buildAssetLinkageDeterministicCheck(contextPackage: any, chapterText: string) {
  const risks = [
...
function stateTrackingContractForSync(contextPackage: any = {}, chapter: any = {}) {
...
function intentConfirmationContractForSync(contextPackage: any = {}, chapter: any = {}) {
  return buildIntentConfirmationContract(contextWithChapterRawPreDraftForSync(contextPackage, chapter)) || {}
}

function normalizeIntentStructureCheck(values: any[], contextPackage: any, chapterText: string) {
...
function normalizeIntentCostRewardCheck(contract: any, contextPackage: any, chapterText: string) {
...
function continuityHeatContractForSync(contextPackage: any = {}, chapter: any = {}) {
...
function conflictStructureContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
...
function upgradeRhythmContractForSync(project: any = {}, contextPackage: any = {}, chapter: any = {}) {
...
function targetReaderContractForSync(project: any, contextPackage: any, chapter: any = {}) {
  return buildTargetReaderContract(project, contextWithChapterRawPreDraftForSync(contextPackage, chapter))
}

function targetReaderArray(values: any) {
  return asArray(values).map((item: any) => compactBriefText(item)).filter(Boolean)
}

function countTargetReaderSignals(chapterText: string, patterns: RegExp[]) {
  const text = String(chapterText || '')
  return patterns.reduce((count, pattern) => count + (pattern.test(text) ? 1 : 0), 0)
}

function normalizeTargetReaderProfileCheck(value: any, chapterText: string) {
...
function normalizeTargetReaderDesireCheck(values: any[], chapterText: string) {
...
function normalizeTargetReaderEmotionalGapCheck(values: any[], chapterText: string) {
  const planned = targetReaderArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
  const scored = planned.map(item => ({ text: item, match: anchorMatchScore(item, chapterText) }))
...
function normalizeTargetReaderAttractionCheck(values: any[], chapterText: string) {
  const planned = targetReaderArray(values)
  if (!planned.length) return null
  const scored = planned.map(item => ({ text: item, match: anchorMatchScore(item, chapterText) }))
  const text = String(chapterText || '')
...
function normalizeTargetReaderCorrectionMethodCheck(values: any[], chapterText: string) {
  const planned = targetReaderArray(values)
  if (!planned.length) return null
  const text = String(chapterText || '')
...
function buildTargetReaderDeterministicCheck(chapterText: string) {
  const text = String(chapterText || '')
  const risks = [
...
function genrePositioningContractForSync(project: any, contextPackage: any, chapter: any = {}) {
...
function plotDynamicsContractForSync(contextPackage: any, chapter: any = {}) {
...
function sceneDriveExpectation(contextPackage: any, chapter: any = {}) {
...
function storyLoopContractFromContext(contextPackage: any = {}, chapter: any = {}) {
  const target = contextPackage?.chapter_target || {}
  const brief = {
    ...(target?.pre_draft_brief || {}),
    ...(target?.preDraftBrief || {}),
...
function informationFlowContractForSync(contextPackage: any = {}, chapter: any = {}) {
  const target = contextPackage?.chapter_target || {}
  const brief = {
    ...(target?.pre_draft_brief || {}),
    ...(target?.preDraftBrief || {}),
...
function expectationThresholdContractForSync(contextPackage: any = {}, chapter: any = {}) {
  const target = contextPackage?.chapter_target || {}
  const brief = {
    ...(target?.pre_draft_brief || {}),
    ...(target?.preDraftBrief || {}),
...
function emotionalArcContractForSync(contextPackage: any = {}, chapter: any = {}) {
  const target = contextPackage?.chapter_target || {}
  const brief = {
    ...(target?.pre_draft_brief || {}),
    ...(target?.preDraftBrief || {}),
...
function millionWordRunwayFromContext(contextPackage: any = {}, preDraftBrief: any = null) {
...
function runwayFromContext(contextPackage: any, chapter: any = {}) {
  const syncContextPackage = contextWithChapterRawPreDraftForSync(contextPackage, chapter)
