...
function sanitizeSceneCardCoreDramaFields(card: any) {
  ...
  return {
    ...
    action: normalizeSceneCardCoreDramaText(card?.action || card?.protagonist_action || card?.protagonistAction, 220),
    turn: normalizeSceneCardCoreDramaText(card?.turn || card?.turning_point || card?.turningPoint, 220),
    payoff: normalizeSceneCardCoreDramaText(card?.payoff || card?.reader_payoff || card?.readerPayoff, 220),
    state_delta: normalizeSceneCardCoreDramaText(card?.state_delta || card?.stateDelta, 220),
...
export function normalizeSceneCardsPayload(payload: any, contextPackage: any = {}) {
  ...
  const outlineCards = directCards.length
    ...
    : asArray(payload?.chapter_outlines)
      ...
      .map((outline: any, index: number) => ({
        ...
        action: outline?.action || outline?.protagonist_action || outline?.protagonistAction || '',
        turn: outline?.turn || outline?.turning_point || outline?.turningPoint || '',
        payoff: outline?.payoff || outline?.reader_payoff || outline?.readerPayoff || '',
        state_delta: outline?.state_delta || outline?.stateDelta || '',
  ...
  const normalizedCards = cards.map((card: any, index: number) => sanitizeSceneCardCoreDramaFields({
    ...
    action: String(card?.action || card?.protagonist_action || card?.protagonistAction || ''),
    turn: String(card?.turn || card?.turning_point || card?.turningPoint || ''),
    payoff: String(card?.payoff || card?.reader_payoff || card?.readerPayoff || ''),
    state_delta: String(card?.state_delta || card?.stateDelta || ''),
