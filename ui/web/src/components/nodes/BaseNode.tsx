...
import { Typography, ColorPicker, ConfigProvider, theme, Input, Tooltip, message } from 'antd'
    ...
    <div className="comfyforge-node-container" style={{ width: '100%', height: '100%', background: bgColor, backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: nodeMuted ? '2px dashed #94a3b8' : borderStyle, borderRadius: 12, boxShadow: nodeMuted ? 'none' : glowShadow, display: 'flex', flexDirection: 'column', position: 'relative', transition: 'border-color 0.3s ease, box-shadow 0.3s ease, opacity 0.3s ease', transform: 'translateZ(0)', opacity: nodeMuted ? 0.5 : 1 }}>
      <div className="custom-drag-handle" style={{ background: `linear-gradient(90deg, ${hexToRgba(nodeColor, 0.1)} 0%, ${hexToRgba(nodeColor, 0.02)} 100%)`, padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${hexToRgba(nodeColor, 0.2)}`, cursor: 'grab', flexShrink: 0, borderTopLeftRadius: 11, borderTopRightRadius: 11 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <Tooltip title="存为资产"><div className="nodrag" onClick={handleSaveAsAsset} style={{ cursor: savingNodeAsset ? 'wait' : 'pointer', padding: 5, lineHeight: 0, background: 'rgba(0,0,0,0.04)', borderRadius: 6, border: '1px solid rgba(0,0,0,0.06)' }}><SaveOutlined style={{ color: savingNodeAsset ? '#0ea5e9' : '#64748b', fontSize: 12 }} /></div></Tooltip>
          {props.onOpenConfig && <Tooltip title="节点配置"><div className="nodrag" onClick={props.onOpenConfig} style={{ cursor: 'pointer', padding: 5, lineHeight: 0, background: 'rgba(0,0,0,0.04)', borderRadius: 6, border: '1px solid rgba(0,0,0,0.06)' }}><SettingOutlined style={{ color: '#64748b', fontSize: 12 }} /></div></Tooltip>}
          <Tooltip title={collapsed ? '展开' : '折叠'}><div className="nodrag" onClick={handleToggleCollapse} style={{ cursor: 'pointer', padding: 5, lineHeight: 0, background: 'rgba(0,0,0,0.04)', borderRadius: 6, border: '1px solid rgba(0,0,0,0.06)' }}>{collapsed ? <ExpandOutlined style={{ color: '#64748b', fontSize: 12 }} /> : <CompressOutlined style={{ color: '#64748b', fontSize: 12 }} />}</div></Tooltip>
