...
import { ApiOutlined, AppstoreAddOutlined, DeleteOutlined, EditOutlined, EyeOutlined, FileTextOutlined, FilterOutlined, PictureOutlined, PlusOutlined, SaveOutlined, SearchOutlined, SendOutlined, VideoCameraOutlined } from '@ant-design/icons'
...
function normalizeProjectList(data: any): ProjectSummary[] {
  const rows = Array.isArray(data?.projects) ? data.projects : Array.isArray(data) ? data : []
  return rows
    .map((item: any) => ({ id: Number(item?.id), name: String(item?.name || '未命名项目') }))
    .filter((item: ProjectSummary) => Number.isFinite(item.id))
}
...
function AssetItem({ asset, onAddToCanvas, onView, onEdit, onDelete }: { asset: Asset; onAddToCanvas?: (asset: Asset) => void; onView?: (asset: Asset) => void; onEdit?: (asset: Asset) => void; onDelete?: (asset: Asset) => void }) {
  ...
  return <div ref={drag} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ padding: 12, borderRadius: 16, border: '1px solid rgba(148,163,184,0.16)', background: isDragging ? '#e0f2fe' : '#fff', boxShadow: '0 10px 28px rgba(15,23,42,0.05)' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      {preview ? <img src={buildAssetMediaUrl(preview)} alt="" style={{ width: 44, height: 44, borderRadius: 12, objectFit: 'cover', border: '1px solid rgba(148,163,184,0.15)' }} /> : <div style={{ width: 44, height: 44, display: 'grid', placeItems: 'center', background: '#f8fafc', borderRadius: 12, color: '#334155' }}>{icon}</div>}
      ...
      {hovered && <Space>
        ...
        {onView && <Button size="small" icon={<EyeOutlined />} onClick={(e) => { e.stopPropagation(); onView(asset) }} />}
...
export default function AssetLibrary({ projectId: initialProjectId, onAddToCanvas }: { projectId?: number; onAddToCanvas?: (asset: Asset) => void }) {
  const navigate = useNavigate()
  const assetsReturnUrl = '/assets'
  ...
  const workflowCreateUrl = `/assets/workflow-config?returnUrl=${encodeURIComponent(assetsReturnUrl)}${selectedProjectId ? `&projectId=${selectedProjectId}` : ''}`
  const workflowEditUrl = editingAsset?.type === 'workflow'
    ? `/assets/workflow-config/edit/${editingAsset.id}?returnUrl=${encodeURIComponent(assetsReturnUrl)}`
    : ''

  useEffect(() => {
    projectApi.getAll().then(res => setProjects(normalizeProjectList(res.data))).catch(() => {
  ...
  }, [initialProjectId, setScope])

  useEffect(() => { fetchAssets(selectedProjectId) }, [fetchAssets, selectedProjectId, scope])
  ...
  const buildEditData = (values: any) => {
    ...
    if (editingAsset.type === 'prompt') return { content: values.content || '' }
  ...
  const viewAsset = (asset: Asset) => {
    if (asset.type === 'workflow') {
      navigate(`/assets/workflow-config/view/${asset.id}?returnUrl=${encodeURIComponent(assetsReturnUrl)}`)
      return
    }
    navigate(`/assets/${asset.id}`)
  }
  const saveEdit = async () => { if (!editingAsset) return; const values = await editingForm.validateFields(); await updateAsset(editingAsset.id, { name: values.name, description: values.description || '', tags: String(values.tags || '').split(/[,，]/).map((s: string) => s.trim()).filter(Boolean), data: buildEditData(values) }); message.success('资产已更新'); setEditingAsset(null) }
  const saveCreate = async () => { const values = await createForm.validateFields(); await createAsset({ type: createType, name: values.name, description: values.description || '', tags: String(values.tags || '').split(/[,，]/).map((s: string) => s.trim()).filter(Boolean), project_id: scope === 'project' ? selectedProjectId || null : null, data: createType === 'prompt' ? { content: values.content || '' } : createType === 'workflow' ? { workflow_json: values.workflow_json ? JSON.parse(values.workflow_json) : {}, parameters: values.parameters ? JSON.parse(values.parameters) : {} } : {} }); message.success('资产已创建'); setCreateOpen(false); createForm.resetFields() }

  return <div style={{ padding: 32 }}>
    <Card style={{ borderRadius: 26, boxShadow: '0 20px 56px rgba(15, 23, 42, 0.08)', overflow: 'hidden' }} styles={{ body: { padding: 24 } }}>
      <Row justify="space-between" align="middle" gutter={16} style={{ marginBottom: 20 }}>
        ...
        <Col>
          <Space>
            <Button icon={<PictureOutlined />} style={{ borderRadius: 12 }} onClick={() => navigate('/assets/create')}>完整铸造</Button>
            <Button type="primary" icon={<PlusOutlined />} style={{ borderRadius: 12 }} onClick={() => { setCreateType('prompt'); setCreateOpen(true) }}>快速新建</Button>
          </Space>
        </Col>
      </Row>

      <Card variant="borderless" style={{ borderRadius: 18, background: 'linear-gradient(180deg, rgba(248,250,252,0.96), rgba(255,255,255,0.98))', marginBottom: 20 }} styles={{ body: { padding: 16 } }}>
        <Space direction="vertical" style={{ width: '100%' }} size={14}>
          <Segmented
            ...
            value={scope === 'global' ? 'global' : selectedProjectId}
            onChange={value => {
              if (value === 'global') {
                setSelectedProjectId(undefined)
                setScope('global')
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {loading ? <div style={{ textAlign: 'center', padding: 24 }}><Spin /></div> : filteredAssets.length ? filteredAssets.map(asset => <AssetItem key={asset.id} asset={asset} onAddToCanvas={onAddToCanvas} onView={viewAsset} onEdit={openEdit} onDelete={async (a) => { await deleteAsset(a.id); message.success('已删除') }} />) : <Card variant="borderless" style={{ borderRadius: 18 }}><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Space direction="vertical"><span>未找到相关资产</span><Button type="primary" size="small" onClick={() => { setCreateType('prompt'); setCreateOpen(true) }}>新建资产</Button></Space>} /></Card>}
      </div>
    </Card>
    ...
    <Drawer title="铸造新资产" open={createOpen} onClose={() => setCreateOpen(false)} width={520} extra={<Button type="primary" icon={<SaveOutlined />} onClick={saveCreate}>创建</Button>}>
      <Form form={createForm} layout="vertical">
        <Form.Item label="资产类型"><Segmented block value={createType} onChange={v => setCreateType(v as any)} options={quickCreateTypeOptions} /></Form.Item>
