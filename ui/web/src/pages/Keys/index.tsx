...
export default function KeyManager() {
  ...
  const providerOptions = dbProviders.filter(p => p.service_type === serviceType).map(p => ({ label: p.display_name, value: p.id }))
  ...
  useEffect(() => { (async () => {
    try {
      const res = await providerApi.getAll(); setDbProviders(res.data)
    } catch { message.error('获取提供商列表失败') }
    fetchKeys()
  })() }, [])

  const handleSyncModels = async (record: APIKey) => {
    if (!record.is_active) return message.warning('请先启用该 Key 后再尝试同步')
    ...
    try {
      ...
      if (drawerVisible && currentKeyForModels?.id === record.id) fetchModels(record.id)
    } catch (error: any) {
      message.error(error.response?.data?.error || error.response?.data?.detail || '同步失败，请检查 API Key 权限或网络')
  ...
  const modelColumns = [
    { title: '常用', dataIndex: 'is_favorite', width: 60, align: 'center' as const, render: (isFav: boolean, record: any) => <div onClick={async () => { try { await modelApi.toggleFavorite(record.id, !isFav); fetchModels(currentKeyForModels!.id) } catch { message.error('状态切换失败') } }} style={{ cursor: 'pointer', transition: 'transform 0.2s' }}>{isFav ? <StarFilled style={{ color: '#faad14', fontSize: 18 }} /> : <StarOutlined style={{ color: '#d9d9d9', fontSize: 18 }} />}</div> },
    ...
    { title: '操作', key: 'action', render: (_: any, record: any) => <Space size="middle"><a onClick={() => handleTestModel(record)}>{testingModel === record.id ? <Spin size="small" /> : '单点测试'}</a>{ENABLE_ADVANCED_PARAM_EDIT && <ModelParamEditor modelId={record.id} modelName={record.model_name} initialParams={record.context_ui_params} onSuccess={() => fetchModels(currentKeyForModels!.id)} />}<a onClick={() => openModelModal(record)}>编辑标签</a>{record.is_manual && <Popconfirm title="确定删除这个模型吗？" onConfirm={() => handleDeleteModel(record.id)}><a style={{ color: 'red' }}>删除</a></Popconfirm>}</Space> },
  ]
  ...
  return <div style={{ padding: 32 }}>
    ...
    <Drawer title={`${currentKeyForModels?.provider || ''} - 模型库管理`} width={850} onClose={() => setDrawerVisible(false)} open={drawerVisible} extra={<Space>{ENABLE_ADVANCED_PARAM_EDIT && <Button type="dashed" icon={<SettingOutlined />} onClick={() => setBulkModalVisible(true)}>批量参数配置</Button>}<Button type="primary" icon={<PlusOutlined />} onClick={() => openModelModal()}>手动添加模型</Button></Space>}>
      <div style={{ marginBottom: 16 }}><Input prefix={<SearchOutlined style={{ color: '#bfbfbf' }} />} placeholder="输入展示名称或模型代号进行过滤..." allowClear onChange={e => setSearchText(e.target.value)} style={{ width: 300, borderRadius: 6 }} /></div>
