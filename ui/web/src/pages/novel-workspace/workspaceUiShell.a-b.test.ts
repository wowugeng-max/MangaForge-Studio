...
describe('commercial writing workspace UI shell a b', () => {
  ...
  test('keeps the left production guide as a compact status summary', () => {
    ...
    expect(component).toContain('交付导出')
    expect(component).toContain('onOpenExportDelivery')
    expect(component).toContain('ExportOutlined')
    expect(component).toContain('任务中心')
    expect(component).not.toContain('主功能区')
    expect(component).not.toContain('onOpenProductionDesk')
    expect(component).not.toContain('AppstoreOutlined')
    ...
    expect(css).toContain('.production-guide-summary-actions')
    expect(css).not.toContain('.production-guide-step-active')
    expect(css).not.toContain('.production-guide-secondary-command')
  })

  test('opens delivery export from the project progress summary', () => {
    const sidebar = source('ChapterDirectorySidebar.tsx')
    const body = source('shell/workspace-body.tsx')
    const bodyProps = source('shell/workspace-view-props-body.ts')

    expect(sidebar).toContain('onOpenExportDelivery')
    expect(sidebar).toContain('onOpenExportDelivery={onOpenExportDelivery}')
    expect(sidebar).not.toContain('onOpenProductionDesk')
    expect(body).toContain('setExportDeliveryOpen')
    expect(body).toContain('onOpenExportDelivery={() => setExportDeliveryOpen(true)}')
    expect(body).not.toContain('onOpenProductionDesk')
    expect(body).not.toContain('navigate(`/novel/workspace/${projectId}/production`)')
    expect(bodyProps).toContain('setExportDeliveryOpen')
  })
