...
describe('project settings editor revision timeout', () => {
  ...
  test('settings source has 内核合同 and GitHub install does not start adapt_pack', () => {
    ...
    expect(modal).toContain('getArtifactContent')
    expect(modal).toContain('loadAdaptContractPreviews')
    expect(kernelBlock).toContain('adaptPackCancelVisible')
    expect(kernelBlock).toContain('正在查看适配进度')
    expect(kernelBlock).toMatch(/adaptPackCancelVisible\([^)]*\)[\s\S]*?取消/)
    expect(kernelBlock).not.toMatch(/phase === 'running' && \(\s*<Button[^>]*>取消/)
    expect(kernelBlock).toContain('item.id')
    expect(kernelBlock).toContain('item.verb')
    expect(kernelBlock).toContain('item.label')
    expect(kernelBlock).not.toContain('item.rel_path || item.id')
